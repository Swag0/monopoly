...
module.exports = {
	...
	execute(message, args, keyv, games) {
		...
		if (message.mentions.users) {
			...
			games.push(new Match(matchCode, [new Player(message.author.id, 0)], 0, "Roll")); //Adds player 1

			for (let i = 0; i < message.mentions.users.array().length; i++) { //Add other players 
				//console.log(message.mentions.users.array()[i].id);
				games[matchCode].players.push(new Player(message.mentions.users.array()[i].id, 0));
			}
		}
